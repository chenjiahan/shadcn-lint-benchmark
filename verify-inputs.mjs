...
for(const p of walk('fixture').filter(p=>/\.(tsx?|css)$/.test(p)||p.endsWith('components.json'))){
 const rel=path.relative('fixture',p);
 assert.equal(hash(p),hash(path.join('.upstream/packages/evals/fixture-rich',rel)),p);
...
for(const p of walk('plugin').filter(p=>p.endsWith('.js'))){
 assert.equal(hash(p),hash(path.join('.upstream/packages/lint/dist',path.basename(p))),p);
 hashes[p]=hash(p);
}
...
for(const dir of ['fixture','stress-fixture','canary-fixture']){
 assert.deepEqual(JSON.parse(fs.readFileSync(`${dir}/tsconfig.json`)),JSON.parse(fs.readFileSync('fixture/tsconfig.json')));
 for(const mode of ['ts','combined']){
  ...
  const normalized=Object.fromEntries(Object.entries(ox).map(([k,v])=>[k.replace(/^typescript\//,'@typescript-eslint/'),v]));
  assert.deepEqual(es,rs);assert.deepEqual(es,normalized);
  assert.equal(Object.keys(es).length,mode==='ts'?23:29);
  for(const [k,v] of Object.entries(ts))assert.equal(es[k],v);
 }
}
fs.writeFileSync('input-check.json',JSON.stringify({upstream:revision,sourceHashes:hashes,stressCopies:1000,stressCopySha256:sourceHash,ruleCounts:{ts:23,combined:29},passed:true},null,2));
console.log('PASS: upstream source, compiled plugin, 1,000 identical copies, and rule configurations');
